...
func callDoubleStream(client echo.EchoClient) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	c, err := client.DoubleStream(ctx)
	if err != nil {
		return
	}
	for i := 0; i < 5; i++ {
		if err := c.Send(&echo.Request{Message: fmt.Sprintf("Request %d", i+1)}); err != nil {
...
func main() {
	...
	conn, err := grpc.Dial(*addr,
		...
		grpc.WithStreamInterceptor(streamInterceptor), grpc.WithBlock())
	if err != nil {
