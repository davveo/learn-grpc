...
const (
	...
	exampleServiceName = "resolver.example.com"
	backendAddr        = "localhost:50051"
)

func callSimple(c echo.EchoClient, message string) {
...
func makeRPCs(cc *grpc.ClientConn, n int) {
	...
	for i := 0; i < n; i++ {
		callSimple(hwc, "this is examples/name_resolving")
	}
}
...
func main() {
	...
	exampleConn, err := grpc.Dial(
		fmt.Sprintf("%s:///%s", exampleScheme, exampleServiceName), // Dial to "example:///resolver.example.com"
		grpc.WithInsecure(),
		grpc.WithBlock(),
	)
	if err != nil {
